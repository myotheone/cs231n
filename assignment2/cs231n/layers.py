...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        now_var = np.mean((x - now_mean.reshape((-1, D)))**2, axis=0)

        batch_normal_x = (x - now_mean.reshape((-1, D))) / (eps + np.sqrt(now_var.reshape((-1, D))))
        out = batch_normal_x * gamma + beta

        cache = (out, batch_normal_x, now_mean, now_var, gamma, beta, bn_param)
        ...
        #######################################################################
        batch_normal_x = (x - running_mean.reshape((-1, D))) / (np.sqrt(running_var.reshape((-1, D))) + eps)
        out = batch_normal_x * gamma + beta
        pass
        #######################################################################
        #                          END OF YOUR CODE                           #
